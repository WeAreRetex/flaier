...
export async function getFlowSpecById(
  event: H3Event,
  flowIdRaw: string | undefined,
): Promise<FlowSpec> {
...
function normalizeManifest(payload: Record<string, unknown>, baseDir: string): FlowManifest {
  ...
  for (const entry of entries) {
    ...
    const fallbackId =
      typeof source === "string" ? createIdFromPath(source) : `flow-${flows.length + 1}`;
...
function getSpecMetadata(spec: FlowSpec) {
  ...
  const rootProps = isObject(rootElement) && isObject(rootElement.props) ? rootElement.props : null;
