...
if (!input) {
  throw new Error("Usage: pnpm run validate -- <spec-or-manifest.json>");
}
...
async function validateManifest(
  ...
  console.log(
    `Validated manifest with ${flowIds.size} flows (${totalValidatedSpecs} local specs checked).`,
  );
}
...
function printUsage() {
  console.log(
    [
      ...
      "",
      "Usage:",
      "  pnpm run validate -- ./flow-specs/manifest.json",
      "  pnpm run validate -- ./flow-specs/auth-login.flow.json",
    ].join("\n"),
  );
}
