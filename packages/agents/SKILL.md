...
description: Generate, repair, and validate flow-visualizer ready flows for flow-narrator. Use when converting code paths into `*.flow.json` specs, scaffolding new flow files, fixing invalid flow specs, adding branching narrative paths, modeling architecture diagrams, tuning specs for docs or Slidev embeds, or producing/updating `manifest.json` for disk-based viewer workflows.
---
