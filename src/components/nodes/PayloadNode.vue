...
<template>
  ...
  >
    ...
    <div class="flex items-center justify-between gap-2 border-b border-border/50 px-3 py-2">
      ...
      <span class="rounded-full border border-border/60 bg-muted/45 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
        {{ format }}
      </span>
    </div>
    ...
    <div class="px-3 py-2.5">
      ...
      <div v-if="hasDiff" class="space-y-2">
        <div v-if="hasBefore" class="rounded-md border border-border/65 bg-muted/30 p-2">
          <p class="mb-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Before</p>
          <pre class="nowheel nopan max-h-[132px] overflow-auto text-[11px] leading-relaxed text-foreground whitespace-pre"><code>{{ before }}</code></pre>
        </div>
        ...
        <div v-if="hasAfter" class="rounded-md border border-primary/30 bg-primary/8 p-2">
          ...
          <pre class="nowheel nopan max-h-[132px] overflow-auto text-[11px] leading-relaxed text-foreground whitespace-pre"><code>{{ after }}</code></pre>
        </div>
      </div>

      <div v-else class="rounded-md border border-border/65 bg-muted/25 p-2">
        <pre class="nowheel nopan max-h-[220px] overflow-auto text-[11px] leading-relaxed text-foreground whitespace-pre"><code>{{ singlePayload }}</code></pre>
      </div>
    </div>
