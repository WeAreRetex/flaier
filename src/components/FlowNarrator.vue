...
function normalizeManifest(manifest: FlowNarratorManifest, baseUrl?: string) {
  ...
  const defaultFlowId = typeof manifest.defaultFlowId === 'string'
    && seenIds.has(manifest.defaultFlowId)
...
function normalizeManifestEntry(entry: FlowNarratorManifestFlow, baseUrl?: string): ResolvedFlowDocument | null {
...
async function fetchJsonSource(source: string) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      return {
        payload,
        resolvedUrl: resolveRelativeSource(candidate, typeof window !== 'undefined' ? window.location.href : undefined),
      }
    } catch (error) {
      lastError = error instanceof Error
        ? error
        : new Error(`Failed to load source "${candidate}".`)
    }
  }
